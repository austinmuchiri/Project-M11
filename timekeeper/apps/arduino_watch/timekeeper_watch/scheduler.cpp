// Routine playback engine. Owns the in-RAM list of routines, walks the clock
// once per second, fires the Reminder screen at scheduledTime, transitions to
// Active on START, Confirm/Reward on hold-DONE, Missed after expectedMinutes.
//
// Ported from ESP-IDF scheduler.c:
//   esp_log_timestamp() → millis()
//   ESP_LOGI()          → Serial.printf()
...
typedef enum { S_IDLE, S_REMINDED, S_ACTIVE, S_PAUSED, S_DONE } sched_state_t;
static sched_state_t s_state = S_IDLE;

void scheduler_init(void)
{
    // Seed a default Morning routine before the phone has synced.
    ...
    static const struct {
        ...
        int min; int stars; int hh; int mm;
    } seed[] = {
...
{
    (void)json; (void)len;
    // TODO: parse JSON into s_routines[] using ArduinoJson or a minimal jsmn port.
    Serial.printf("[sched] TODO parse %u-byte routine payload\n", (unsigned)len);
}
    ...
    for (int ri = 0; ri < s_routine_count; ri++) {
        ...
        for (int ti = 0; ti < r->task_count; ti++) {
            ...
            if (t->scheduled_min_of_day == now_min_of_day) {
                ...
                Serial.printf("[sched] remind: %s/%s\n", r->id, t->id);
                return;
            }
        }
    }
...
static void emit_event(const char *status)
{
    ...
    snprintf(buf, sizeof(buf),
        ...
        "\"source\":\"watch\",\"ts\":%lld}",
        r->id, t->id, status, (long long)millis());
...
void scheduler_start_current(void)
{
    ...
    s_state              = S_ACTIVE;
    s_active_started_min = (int)(millis() / 60000UL);
    emit_event("started");

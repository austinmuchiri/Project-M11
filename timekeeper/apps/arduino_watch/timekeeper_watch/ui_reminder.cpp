// 03 · Reminder — slow breathe animation + START button
...
{
    (void)e;
    // scheduler_start_current is wired via a forward declaration in scheduler.cpp.
    extern void scheduler_start_current(void);
    scheduler_start_current();
}
...
lv_obj_t *ui_reminder_create(void)
{
    ...
    lv_obj_add_event_cb(btn, on_start, LV_EVENT_CLICKED, NULL);

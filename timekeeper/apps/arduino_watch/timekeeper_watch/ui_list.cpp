// 02 · Today list — scrollable task rows
...
static void add_row(lv_obj_t *parent, const list_row_t *r)
{
    ...
    lv_obj_set_style_bg_color(row,
        r->status == ROW_ACTIVE ? COLOR_BRAND_SOFT : COLOR_SURFACE, 0);
    lv_obj_set_style_bg_opa(row, LV_OPA_COVER, 0);
    lv_obj_set_style_border_width(row, r->status == ROW_ACTIVE ? 2 : 1, 0);
    lv_obj_set_style_border_color(row,
        r->status == ROW_ACTIVE ? COLOR_BRAND : COLOR_INK_FAINT, 0);
    lv_obj_set_style_border_opa(row,
        r->status == ROW_ACTIVE ? LV_OPA_COVER : LV_OPA_30, 0);

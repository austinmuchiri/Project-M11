// 07 · Missed — gentle retry · Skip / Try Again
#include "ui_theme.h"

extern void scheduler_skip_current(void);
extern void scheduler_retry_current(void);

static void on_skip(lv_event_t *e)  { (void)e; scheduler_skip_current();  ui_show_screen(SCR_HOME); }
static void on_retry(lv_event_t *e) { (void)e; scheduler_retry_current(); ui_show_screen(SCR_REMINDER); }
...
lv_obj_t *ui_missed_create(void)
{
    ...
    lv_label_set_text(body, "Brush Teeth · still on the list");

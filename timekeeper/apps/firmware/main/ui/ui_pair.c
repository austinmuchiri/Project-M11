// 08 · BLE pairing — 4-digit code + spinner
#include "ui_theme.h"

extern const char *ble_pair_code(void);
...
lv_obj_t *ui_pair_create(void)
{
    ...
    lv_obj_set_style_arc_color(spin, lv_color_hex(0x00000000), LV_PART_MAIN);
    ...
    // Center BLE glyph
    lv_obj_t *ble = lv_label_create(halo);
    lv_label_set_text(ble, LV_SYMBOL_BLUETOOTH);
    lv_obj_set_style_text_color(ble, COLOR_BRAND, 0);
    lv_obj_set_style_text_font(ble, &font_nunito_96, 0);
    lv_obj_center(ble);
